...
**稳定版未变化。** E2B Infra `2026.29`、KServe `v0.20.0`、GPU Operator `v26.7.0@10ee5b3638b89e11e949412aafa5ba99279c3721`、HAMi `v2.10.0@4707fb02c91c545bc7343ce26dba4c32919f9a3e`、Kubeflow Community Distribution `26.03.1`、Koordinator `v1.8.0`、Kueue `v0.19.2@8eab68778fc1b52affe165fdf5af29d1e9b4f3cb`、KAI-Scheduler `v0.17.0`、SGLang `v0.5.18@71de97b264b04dcd514cf904003028aefe9775c8`、AIPerf `v0.12.0`、GuideLLM `v0.7.3`、inference-perf `v0.6.1`、genai-bench `v0.0.5`、LLMPerf `v2.0` 与 ollama-benchmark `v0.5.2` 经复核保持原基线。Kueue v0.19.2 仍不能跳过 v0.19.0 的 API、feature gate、Ray 配额和 Helm 清理前置；KAI 虽有 tag-only `v0.20.1@5922dc7d1a4661d3fc43d60943f92a775c892bdc`，但没有对应正式 GitHub Release，因此不替代 v0.17.0。
...
| [Multica-Deep-Dive.md](Multica-Deep-Dive.md) | `v0.4.37` | AI-native 团队任务管理与 coding agent 编排平台 | 关注 coding agent 团队协作、本地 CLI 执行、自托管和权限治理的工程师 | Issue/Task、25 种 protocol family、Plugin Public API v1、ZeroClaw/CodeArts、Runtime claim、进程树/daemon 恢复、Squad、Autopilot 与无 Sandbox 边界 |
| [LLM-Benchmark-Deep-Dive.md](LLM-Benchmark-Deep-Dive.md) | AIPerf `v0.12.0`；GuideLLM `v0.7.3`；SGLang `v0.5.18`；vLLM `v0.28.0`；EvalScope `v1.11.1`；其余见正文 | LLM 压测工具深度对比 | 需要选择或设计推理压测方案的工程师 | AgentX、adaptive scale、SGLang 缓存目录/依赖迁移、vLLM dataset/timed trace、EvalScope warmup/SSE/指标正确性与多工具对比 |
...
| [Kubernetes-Blog-Feature-Deep-Dive.md](Kubernetes-Blog-Feature-Deep-Dive.md) | Kubernetes v1.37.0；RSS 截止 2026-09-05 | Kubernetes Blog/RSS 跨 SIG 新特性综述 | 需要理解 Kubernetes v1.36/v1.37 新 API、成熟度、升级和 AI 平台组合的工程师 | HPA Scale-to-Zero、DRA、Gang Scheduling、Storage Version Migration、RangeStream、证书、rootless、KYAML、Gateway API、Agent Sandbox |
...
| [Kubernetes-AI-Schedulers-Deep-Dive.md](Kubernetes-AI-Schedulers-Deep-Dive.md) | Koordinator `v1.8.0`；Kueue `v0.19.2`；Grove `alpha.12`；KAI `v0.17.0`；Volcano `v1.15.2` | Kubernetes AI 调度器深度对比 | 负责 GPU 集群、训练/推理平台、批任务队列和调度器选型的平台工程师 | Kueue TAS/DRA/FairSharing、Grove PodGangMap/KAI backend、KAI tag-only 边界、Volcano GHSA 修复、组合架构与升级治理 |
...
- Kubernetes AI 平台：按 [Kubernetes-Blog-Feature-Deep-Dive.md](Kubernetes-Blog-Feature-Deep-Dive.md) → [Kubernetes-Native-Scheduler-Deep-Dive.md](Kubernetes-Native-Scheduler-Deep-Dive.md) → [Kubernetes-AI-Schedulers-Deep-Dive.md](Kubernetes-AI-Schedulers-Deep-Dive.md) → [Volcano-Upgrade-Compatibility-Deep-Dive.md](Volcano-Upgrade-Compatibility-Deep-Dive.md) → [KServe-Deep-Dive.md](KServe-Deep-Dive.md) / [Kubeflow-Deep-Dive.md](Kubeflow-Deep-Dive.md) 阅读；需要节点设备和共享隔离细节时，再补读 [NVIDIA-GPU-Operator-Deep-Dive.md](NVIDIA-GPU-Operator-Deep-Dive.md) 与 [HAMi-Deep-Dive.md](HAMi-Deep-Dive.md)。这条路径依次覆盖 Blog/RSS 新特性、原生调度基线、AI 队列与调度扩展、Volcano 升级兼容、模型推理服务以及上层 MLOps 平台。
